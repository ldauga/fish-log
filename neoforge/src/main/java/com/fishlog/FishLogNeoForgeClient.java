...
/** Logique client NeoForge — uniquement chargé côté client via DistExecutor. */
class FishLogNeoForgeClient {
    ...
    static void init(IEventBus modEventBus) {
        FishLogNeoForgeClient instance = new FishLogNeoForgeClient();
        modEventBus.addListener(instance::onRegisterKeyMappings);
        NeoForge.EVENT_BUS.addListener(instance::onClientTick);
        NeoForge.EVENT_BUS.addListener(instance::onLoggingIn);
        NeoForge.EVENT_BUS.addListener(instance::onSystemChat);
        FishLogCommon.LOGGER.info("[FishLog] Mod chargé (NeoForge)");
    }
